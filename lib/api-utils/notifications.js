...
async function sendAdminOrderSms(order) {
  ...
  if (provider === 'arkesel') {
    ...
    const sender = process.env.ARKESEL_SENDER || 'CKS'
  ...
  const sender = process.env.PAYSTACK_SMS_SENDER || 'CKS Motors'
...
async function sendCustomerOrderSms(order) {
  ...
  const provider = (process.env.SMS_PROVIDER || 'paystack').toLowerCase()
  const message = `Hello ${order.name}, thank you for your order ${order.id} for the ${order.productName || 'vehicle'}. Pickup point: ${order.location || 'to be confirmed'}. We have received your request and will contact you shortly. CKS Motors.`

  try {
    if (provider === 'arkesel') {
      const key = process.env.ARKESEL_API_KEY
      const sender = process.env.ARKESEL_SENDER || 'CKS Motors'
    ...
    const payload = { to: order.phone, from: process.env.PAYSTACK_SMS_SENDER || 'CKS Logistics', message }
    const r = await fetch(paystackSmsUrl, { method: 'POST', headers: { Authorization: `Bearer ${paystackSecretKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
...
async function sendCustomerOrderEmail(order) {
  ...
  if (gmailClientSecret) {
    try {
      ...
      const data = await sendViaGmailApi({
        from: fromEmail,
        to: order.email,
        subject: `Your CKS Rental Confirmation - Order #${order.id}`,
        html: `<p>Hello ${order.name},</p><p>Thank you for choosing CKS Motors. We have received your rental request for the <b>${order.productName}</b> (Order #${order.id}).</p><p>Our team will contact you shortly.</p>`,
        attachments: [
          {
  ...
  const text = `Hello ${order.name},\n\nThank you for choosing CKS Motors. We have received your rental request for the ${order.productName || 'vehicle'} (Order #${order.id}).\n\nOur team will contact you shortly.\n\nBest regards,\nCKS Motors Team`
...
async function sendApprovalSms(order) {
  if (!order.phone) return { ok: false, skipped: true, reason: 'Missing customer phone' }
  const provider = (process.env.SMS_PROVIDER || 'paystack').toLowerCase()
  const message = `Congratulations ${order.name}! Your CKS Motors rental (Order #${order.id}) has been APPROVED. Our chauffeur will be at your location: ${order.location || 'Accra'} at the scheduled time. Link to order: https://ckssite.com/my-orders. Drive in luxury.`
  ...
  if (gmailClientSecret) {
    try {
      ...
      const data = await sendViaGmailApi({
        from: fromEmail,
        to: order.email,
        subject: `Approved: Your CKS Rental Booking - Order #${order.id}`,
        html: `<p>Hello ${order.name},</p><p>We are pleased to inform you that your rental request (Order #${order.id}) for the <b>${order.productName}</b> has been <b>APPROVED</b>.</p><p>Please find your official invoice attached to this email.</p><p>Our team is now finalizing the logistics for your chauffeur and vehicle.</p><p>Scheduled Dates: ${order.start} to ${order.end}</p><p>You can track your orders here: https://ckssite.com/my-orders</p><br/><p>Best regards,<br/>CKS Motors Team</p>`,
        attachments: [
          {
            filename: `invoice-${order.id}.pdf`,
            content: invoiceBuffer
