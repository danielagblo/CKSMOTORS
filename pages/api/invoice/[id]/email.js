...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (gmailClientSecret) {
      try {
        ...
        const data = await sendViaGmailApi({
          from: fromEmail,
          to: order.email,
          subject: `CKS Motors Logistics and transport — Invoice ${order.id}`,
          html: `<p>Dear ${order.name},</p><p>Please find attached the invoice for your order <b>${order.id}</b>.</p><p>Payment will be collected on delivery.</p><p>Thank you,<br/>CKS Motors and transport</p>`,
          attachments: [
            {
    ...
    const fromEmail = settings?.fromEmail || process.env.FROM_EMAIL || smtpUser || "no-reply@cksmotors.com";
